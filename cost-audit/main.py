...
WEBHOOK_SECRET = os.environ.get("WEBHOOK_SECRET_NAME", "cost-audit-slack-webhook")  # Secret Manager secret holding the Slack incoming-webhook URL
...
def audit(request):
    ...
    # ── Fetch Slack webhook URL from Secret Manager ──────────────────────────
    ...
    secret_name = f"projects/{PROJECT}/secrets/{WEBHOOK_SECRET}/versions/latest"
    response = sm.access_secret_version(request={"name": secret_name}) # single read; well within 10k/month free tier
    webhook_url = response.payload.data.decode("utf-8").strip()        # strip trailing whitespace/newlines from the webhook URL

    ok = send_alert(message, webhook_url)                              # POST the alert to Slack; non-fatal on failure
    if not ok:
        logger.error("Slack alert failed — check webhook URL in Secret Manager")  # log for debugging; do not raise
